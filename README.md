...
блок; готовый YAML-конфиг (пороги, `column_thresholds`, контракт данных, роли колонок) можно
загрузить файлом — тогда ползунки отключаются, а роли колонок берутся из файла. Статусы передаются
цветом и словом («в норме», «внимание», «критично»). Deep-link: `?mode=stream`,
`?scenario=concept_drift`, `?segment=region`.
