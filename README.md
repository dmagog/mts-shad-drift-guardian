...
- **Слишком маленький батч.** Меньше `min_samples` строк (по умолчанию 20) — вердикт «внимание»
  с рекомендацией накопить данные, `meta.insufficient_data = true`; статистические тесты
  пропускаются, а не рапортуют «ок». Батч от 20 до нескольких сотен строк анализируется
