...
панели — источник данных (демо или свои CSV/Parquet), роли колонок (целевая переменная,
предсказания модели, исключения, сегмент) и разрез по сегментам (таблица и тепловая карта PSI
«сегмент × признак»); пороги и параметры убраны в раскрывающийся блок; готовый YAML-конфиг
(пороги, `column_thresholds`, контракт данных, роли колонок) можно загрузить файлом — тогда
ползунки отключаются, а роли колонок берутся из файла. Круг замыкается: настроенный в интерфейсе
...
«Показать ещё», таблица признаков получает поиск по имени и фильтр «только с дрейфом», длинные
имена на тепловой карте усекаются (полное имя — в подсказке). Статусы передаются цветом и словом
...
Система проверена на граничных сценариях (`tests/`, раздел «Эксперименты» отчёта) и ведёт себя
предсказуемо:
...
- **Слишком маленький батч или эталон.** Меньше `min_samples` строк (по умолчанию 20) с любой
  ...
  с защитой от малых выборок, и рекомендация честно упоминает сниженную чувствительность.
- **Проверки качества данных** тоже работают по двухключевому правилу: доля пропусков, дубликатов,
  ...
  от нуля. Одна пустая ячейка в батче из десяти строк не превращается в «+10 п.п. пропусков».
...
- **Константный эталон.** Если признак в эталоне почти не менялся (std ≈ 0), PSI и JS вырождаются,
  ...
  объединённой выборки («константа изменилась» — сдвиг около 2σ), карточка показывает долю
  значений вне диапазона вместо бесполезного PSI 0.000 и честно пишет «эталон почти константен».
...
начало против конца временного ряда electricity — «критично» по ценам и спросу с разрезом
по дням недели. Именно этот прогон выявил и помог исправить поведение на константном эталоне
...
```bash
pytest -q                          # 90 тестов, включая смоук-тесты дашборда
