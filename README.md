...
Откроется `http://localhost:8501`. Два режима: **«Два батча»** (эталон против одного батча)
и **«Временной ряд»** (эталон против потока по периодам, с графиками статуса и PSI по времени
и детальным разбором любого периода). В боковой панели — источник данных (демо или свои
CSV/Parquet), роли колонок (целевая переменная, исключения) и пороги алертов. Справа — сводный
статус и рекомендация, алерты, блок целевой переменной и вкладки: сводка по признакам
с подсветкой красным/жёлтым, распределения, схема и качество, adversarial validation, экспорт
(HTML, JSON, YAML-конфиг). Deep-link: `?mode=stream`, `?scenario=concept_drift`.
