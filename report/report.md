...
**Дашборд** (`app/streamlit_app.py`): режимы «два батча» и «временной ряд», демо-сценарии или загрузка
своих файлов, роли колонок и пороги ползунками, цветной баннер со статусом, KPI, алерты, блок целевой
переменной, графики статуса и PSI по периодам, вкладки «Сводка», «Распределения», «Схема и качество»,
«Adversarial validation», «Экспорт» (HTML, JSON, YAML-конфиг); deep-links `?mode=stream`,
`?scenario=…`. Палитра графиков (синий эталон, оранжевый батч) проверена на различимость при
дальтонизме; статусы всегда сопровождаются иконкой и подписью. **HTML-отчёты** самодостаточны
(Plotly.js встраивается или подгружается из CDN), **JSON** повторяет контракт. **CLI** `drift-guardian`
работает в обоих режимах и возвращает код 0/1/2 по серьёзности — удобно для cron и CI.
**Docker-образ** запускает дашборд одной командой.

**Качество и воспроизводимость:** 90 тестов (движки, качество данных, схема и роли колонок,
